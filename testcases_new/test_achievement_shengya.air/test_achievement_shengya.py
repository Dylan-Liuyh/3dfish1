...
def test_achievement_shengya():
    ...
    if poco(texture="lobby_btn_aachievement").exists():
        ...
        #规则说明
        if poco(texture='arena_lobby_btn_rule').exists():
            poco(texture='arena_lobby_btn_rule').click()
            


            poco("achievement_rules").wait_for_appearance()
            poco("achievement_rules").offspring("Button").click()
        #大奖
        if poco('bigReward').exists():
            poco('bigReward').click()
            poco('tips').wait_for_appearance()
            poco('tips').offspring('title').click()
        #点击生涯成就
        pos=poco(text='生涯成就').attr('pos')
        if pos[1]>0.9:
            print('超出屏幕')
